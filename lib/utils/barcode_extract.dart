...
List<TimeOfDay> determineScheduledTimes(String text) {
  text = text.toLowerCase();
  List<TimeOfDay> scheduledTimes = [];
  Iterable<Match> matches =
      RegExp(r'at\s(\d{1,2}(:\d{1,2})(am|pm)?)').allMatches(text);
  for (final Match m in matches) {
    scheduledTimes.add(stringToTimeOfDay(m[0]!));
  }
  return scheduledTimes;
}

Dosage determineMedicineDosage(String text) {
  MedicationFrequency? frequency = determineMedicineFrequency(text);
  int? frequencyCount;
  if (frequency != null && frequency != MedicationFrequency.onceADay && frequency != MedicationFrequency.onlyAsNeeded) {
    frequencyCount = determineMedicineFrequencyCount(text);
  }
  return Dosage(
      frequency: frequency,
      frequencyCount: frequencyCount,
      scheduledTimes: determineScheduledTimes(text),
...
Prescription determinePrescription(String text) {
  int? totalQuantity = determineTotalQuantity(text);
  return Prescription(
    totalQuantity: totalQuantity,
    remainingQuantity: totalQuantity,
    thresholdQuantity: totalQuantity != null ? (totalQuantity * 0.25).ceil(): null,
